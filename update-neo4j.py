...
to_disease = 25
disease_uri = "neo4j+s://disease.ncats.io:7687"
neo4j_uri = "bolt://disease.ncats.io:80"
...
def save_omim_articles(mindate, maxdate):
  ...
  with GraphDatabase.driver(neo4j_uri, encrypted=False) as driver:
    #with driver.session() as session:
...
def create_indexes():
  with GraphDatabase.driver(production_url, encrypted=False) as driver:
    with driver.session() as session:
      cypher_query = [
