#Richard: load (add) publication type to (Article) nodes
...
from_disease = 0
#to_disease = from_disease + 10
to_disease = None

def get_current_pubmed_ids():
  #Returns list of pubmed_id in current neo4j
  with GraphDatabase.driver("bolt://rdip2.ncats.io:7687", encrypted=False) as driver:
    with driver.session() as session:
      cypher_query = '''
      match p = (n:Article) return n.pubmed_id as pubmed_id
      '''
...
def load_pubtype():
  pubmed_ids = get_current_pubmed_ids()
  print(f'Total articles: {len(pubmed_ids)}')
  with GraphDatabase.driver("bolt://rdip2.ncats.io:7687", encrypted=False) as driver:
    with driver.session() as session:
      tx = session.begin_transaction()
      for i in range(0, len(pubmed_ids), 1000):
        sub_ids = pubmed_ids[i:(i+1000)]
        print(f'\n{i} out of {len(pubmed_ids)}')
        ids = ' OR ext_id:'.join(sub_ids)
        ids = 'ext_id:' +ids
        #logging.info(ids)
        ...
        if ('resultList' in abstracts and 'result' in abstracts['resultList'] and len(abstracts['resultList']['result']) > 0):
          for result in abstracts['resultList']['result']:
            ...
            res = tx.run("match(a:Article{pubmed_id:$pmid}) return id(a) as id", pmid = pubmedID)
            record = res.single()
            if (record):
              article_node = record["id"]
              if ('pubTypeList' in result and 'pubType' in result['pubTypeList']):
                #logging.info(f'Invoking add_pubtype')
                add_pubtype(tx, result['pubTypeList']['pubType'], article_node)
            else:
              logging.error(f'PubMed article NOT exists: {pubmedID}, {article_node}')
      tx.commit()
...
def create_indexes():
  with GraphDatabase.driver("bolt://localhost:7687", encrypted=False) as driver:
    with driver.session() as session:
      ...
      "CREATE INDEX IF NOT EXISTS FOR (n:Substance) ON (n.name, n.registryNumber)"
      ]
...
def create_chemicals(tx, abstractDataRel, article_node):
  ...
  MATCH (a:Article) WHERE id(a) = $article_id
  MERGE (u:Substance {name:$name, registryNumber:$registryNumber})
  MERGE (u) - [r:SUBSTANCE_ANNOTATED_BY_PUBMED] -> (a)
  '''
...
def add_pubtype(tx, abstractDataRel, article_node):
  #logging.info(f'\nadd pubType to article: {article_node}, {abstractDataRel}')
  query = '''
  MATCH (a:Article) WHERE id(a) = $article_id
  SET a.pubType = $pubType
  '''
  
  tx.run(query, parameters={
    "article_id":article_node,
    "pubType": abstractDataRel,
  })

def add_pubtype000(tx, abstractDataRel, article_node):
  logging.info(f'\nadd pubType to article: {article_node}, {abstractDataRel}')
