...
def upload_abstracts(dir_path):
    ...
    insert ='INSERT INTO grant_abstract (YEAR, APPLICATION_ID, ABSTRACT_TEXT) VALUES (%s, %s, %s)'
    ...
    for csv_file in csv_files:
        ...
        with open(csv_file, 'r', newline='', encoding=encoding, errors='replace') as file:
            ...
            for row in list(reader):
                data_tuple = ( 
                    year,
                    row['APPLICATION_ID'] if row['APPLICATION_ID'] else None,  # APPLICATION_ID 
                    row['ABSTRACT_TEXT'] if row['ABSTRACT_TEXT'] else None  # ABSTRACT_TEXT
                )  
...
if __name__ == '__main__':
    ...
    ok = ask_to_continue(f'*** Upload the grant Abstracts into MySQL database? *** ')

    if not ok:
        sys.exit(Fore.RED + '\n------------------------ Stopped ------------------------\n'+ Style.RESET_ALL)


    dir = f'{Path(__file__).parent}/data/abstracts'
