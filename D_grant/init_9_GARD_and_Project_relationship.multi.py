...
GARD_TERM_SEPARATOR = "$$$"
LOG_FILE_PATH = PROJECT_ROOT / "logs" / "grant_GARD_Project_relation_process.log"
...
# ClinicalBERT is loaded once per worker process. Keeping the worker count
# conservative avoids loading too many model copies into memory at the same time.
...
def initialize_worker() -> None:
    """Prepare per-process data before a worker starts ID-range processing."""
...
def semantic_similarity(input_text: str, target_term: str) -> float:
    ...
    loads the tokenizer/model once per worker process and reuses them for all
    comparisons in that process.
    """
...
def process_id_range(id_range: Tuple[int, int]) -> Dict[str, int]:
...
def main() -> int:
    """Run the multiprocessing GARD-project relationship pipeline."""
    ...
    print(
        ...
        f"id_ranges={len(id_ranges)}, processes={DEFAULT_NUM_PROCESSES}\n"
    )
    ...
    with Pool(processes=DEFAULT_NUM_PROCESSES, initializer=initialize_worker) as pool:
        for range_summary in pool.imap_unordered(process_id_range, id_ranges, chunksize=1):
    ...
    print(
        ...
        f"projects_scanned={total_summary['projects_scanned']}, "
        f"projects_with_results={total_summary['projects_with_results']}, "
        f"relationships_inserted={total_summary['relationships_inserted']}\n"
    )
