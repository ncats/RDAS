...
'''
--- Batch update, for large number of is_EPI, is_NHS are NULLs ---
...
run_sql = '''
    ...
    WHERE g.Label_Predicate_Mapping LIKE 'ABBRE%'  and pgs.is_abbreviation is null;
'''
...
if __name__ == "__main__": 
    ...
    ok = ask_to_continue('Update is_EPI, is_NHS (default is NULL) in table publication_article?')
    if not ok:
        sys.exit('------Stopped ------')
    ...
    try:
        with db().mysql_conn() as select_conn, \
            ...
            query = 'SELECT pubmed_id FROM publication_article WHERE is_EPI is null'
