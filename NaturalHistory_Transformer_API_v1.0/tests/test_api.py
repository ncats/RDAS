...
"""]
        response = self.app.post('/predict', data=json.dumps({"texts": sample_text}),
    ...
    def test_predict_endpoint_error(self):
        # Define a test case to ensure error handling works (sending wrong data format)
        response = self.app.post('/predict', data=json.dumps({"text": "This will cause an error"}),
                                 content_type='application/json')
        self.assertEqual(response.status_code, 400)

    def test_predict_endpoint_with_invalid_input_type(self):
        # Test sending non-list types in 'texts'
        response = self.app.post('/predict', data=json.dumps({"texts": "This should fail"}),
                                 content_type='application/json')
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.data)
        self.assertIn('error', data)
        self.assertEqual(data['error'], "Input 'texts' must be a non-empty list of strings.")
