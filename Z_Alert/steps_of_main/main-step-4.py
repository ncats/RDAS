...
LOOK_BACK_DAYS = 7
STEP_NAME = "Step 4: run_memgraph_index_initialization()"


def run_disabled_step(runner: AlertPipelineRunner) -> None:
    runner.logger.info(f'{"*" * 30} --- run_memgraph_index_initialization --- is disabled {"*" * 30}\n\n')
...
def main() -> None:
    ...
    try:
        runner._run_step_with_timing(STEP_NAME, lambda: run_disabled_step(runner))
