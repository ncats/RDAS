...
rd_ids = pd.read_csv('../../data_raw/NormMap_mapped_app_ids.csv')
rd_ids = rd_ids['APPLICATION_ID'].tolist()
       
# Get CSV files lists from a folder
input_path = '../../data_neo4j/projects_all_with_funds/'
files = glob.glob(input_path + '*.csv')

for file in files:
    output_file = '../../data_neo4j/projects/RD_PROJECTS_' + file[43:47] + '.csv'
