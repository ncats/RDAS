        ...
        if ('resultList' in abstracts and 'result' in abstracts['resultList'] and len(abstracts['resultList']['result']) > 0):
          for result in abstracts['resultList']['result']:
            ...
            if (record):
              ...
              #logging.info(f'PubMed article already exists: {pubmedID}, {article_node}')
              #create_chemicals(tx, abstractDataRel, article_node)
              if ('chemicalList' in result and 'chemical' in result['chemicalList']):
                #logging.info(f'Invoking create_chemical')
                create_chemicals(tx, result['chemicalList']['chemical'], article_node)
            else:
