...
'''
class OrganizationLocationGraphBatchUpdateTask:
    """
    ...
    5. Merges duplicate Organization nodes that now share the same real ror_id.
       ...
       nodes are rewired to the keeper in batches, and duplicate nodes are
       deleted in batches.
    6. Merges duplicate Location nodes by stable _idx_key using the same batched
    ...
    def update(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"{type(self).__name__} failed: {e}")
...
if __name__ == "__main__":
    OrganizationLocationGraphBatchUpdateTask().update()
