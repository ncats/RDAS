...
5. Download only PubMed articles that are not already in publication_article or update_publication_article.
"""

# Reference: alert/pipelines/pipeline_2/task_clinical_trial_4.py
...
class NewOmimPublicationArticleImportTask(PipelineBase):
    ...
    def process_new_data(self) -> None:
        ...
        try:
            ...
            '''
            Step 5: Import only the missing article metadata into the update
            table. Later publication tasks will classify, graph, and copy these rows.
            '''
    ...
    def _retrieve_missing_publication_articles(self, pubmed_ids):
        ...
        '''
        Download article metadata only for PMIDs missing from both publication
        tables. update_publication_article is the staging table used by the
        downstream alert publication pipeline.
        '''
        pubmed_ids = sorted(
        ...
        ''' Avoid duplicate inserts across both historical and current update tables. '''
        ...
        insert_article_sql = self.publication_worker.get_insert_sql("update_publication_article")
        insert_cursor = None
        inserted_count = 0
        ...
        try:
            ...
            for pubmed_id in missing_pubmed_ids:
                ...
                insert_cursor.execute(insert_article_sql, article_val)
                self.mysql.commit()

                inserted_count += 1
                self.logger.info(f"Inserted OMIM PubMed article into update_publication_article: pubmed_id={pubmed_id}")

            self.logger.info(f"Inserted {inserted_count} OMIM PubMed articles into update_publication_article.")
    ...
    def _get_existing_pubmed_ids_(self, pubmed_ids):

        '''
        Return PMIDs that already exist in either the main publication table or
        the alert staging table. This prevents duplicate article downloads and
        duplicate update_publication_article rows.
        '''
        ...
        try:
            ...
            for batch in self._iter_batches(pubmed_ids, self.BATCH_SIZE):
                ''' The same batch is used once for each side of the UNION. '''
                placeholders = ','.join(['%s'] * len(batch))
                query = f'''
                    SELECT pubmed_id
